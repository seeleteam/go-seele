...
func NewNode(nodeID string) *Node {
	...
	*/
	node := &Node{
		...
		log:         log.GetLogger("PBFTengine_node"),
	}
...
func (node *Node) Reply(msg *consensus.ReplyMsg) error {
	...
	for _, value := range node.CommittedMsgs {
		node.log.Info("Committed value:\n ClientID:[%s], Timestamp:[%d], Operation:[%s], SequenceID:[%d]\n", value.ClientID, value.Timestamp, value.Operation, value.SequenceID)
	}
	// fmt.Print("\n")
...
func (node *Node) GetReq(reqMsg *consensus.RequestMsg) error {
	...
	node.LogStage(fmt.Sprintf(" (ViewID:%d)", node.CurrentState.ViewID), false)

	// Send getPrePrepare message
	if prePrepareMsg != nil {
		node.Broadcast(prePrepareMsg, "/preprepare")
		node.LogStage("Pre-prepare", true)
	}
...
func (node *Node) GetPrepare(prepareMsg *consensus.VoteMsg) error {
	...
	if commitMsg != nil {
		...
		node.LogStage("Prepare", true)
		node.Broadcast(commitMsg, "/commit")
		node.LogStage("Commit", false)
	}
...
func (node *Node) GetCommit(commitMsg *consensus.VoteMsg) error {
	...
	if replyMsg != nil {
		...
		node.LogStage("Commit", true)
		node.Reply(replyMsg)
		node.LogStage("Reply", true)
	}
...
func (node *Node) GetReply(msg *consensus.ReplyMsg) {
	node.log.Info("Result: %s by %s", msg.Result, msg.NodeID)
}
...
func (node *Node) createStateForNewConsensus() error {
	...
	node.LogStage("Create the replica status", true)
...
func (node *Node) LogStage(stage string, isDone bool) {
	if isDone {
		node.log.Info("[STAGE-DONE] %s", stage)
	} else {
		node.log.Info("[STAGE-BEGIN] %s", stage)
	}
}
...
func (node *Node) LogMsg(msg interface{}) {
	...
	case *consensus.RequestMsg:
		...
		node.log.Info("[REQUEST] ClientID: %s, Timestamp: %d, Operation: %s", reqMsg.ClientID, reqMsg.Timestamp, reqMsg.Operation)
	case *consensus.PrePrepareMsg:
		prePrepareMsg := msg.(*consensus.PrePrepareMsg)
		node.log.Info("[PREPREPARE] ClientID: %s, Operation: %s, SequenceID: %d\n", prePrepareMsg.RequestMsg.ClientID, prePrepareMsg.RequestMsg.Operation, prePrepareMsg.SequenceID)
	case *consensus.VoteMsg:
		voteMsg := msg.(*consensus.VoteMsg)
		if voteMsg.MsgType == consensus.PrepareMsg {
			node.log.Info("[PREPARE] NodeID: %s", voteMsg.NodeID)
		} else if voteMsg.MsgType == consensus.CommitMsg {
			node.log.Info("[COMMIT] NodeID: %s", voteMsg.NodeID)
		}
	}
}
