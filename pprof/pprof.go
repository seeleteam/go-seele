...
// Status return the server information
func (p *ProfServer) Status() string {
	p.lock.Lock()
	defer p.lock.Unlock()

	if p.alive {
		return fmt.Sprint("go tool pprof is alive")
	}

	return fmt.Sprint("go tool pprof is not alive")
}

// GetPort return port
func (p *ProfServer) GetPort() uint64 {
	return p.port
}
...
func (p *ProfServer) Start() error {
	...
	// if server is alive return error
	if p.alive {
...
// Stop the go tool pprof http server
func (p *ProfServer) Stop() error {
	p.lock.Lock()
	defer p.lock.Unlock()
