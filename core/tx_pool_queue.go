...
func newPendingQueue() *pendingQueue {
	return &pendingQueue{
		...
		heap: common.NewHeap(func(i, j common.HeapItem) bool {
...
func (q *pendingQueue) add(tx *pooledTx) {
	if collection := q.txs[tx.Data.From]; collection != nil {
		if updated := !collection.add(tx); updated {
			heap.Fix(q.heap, collection.GetHeapIndex())
		}
	} else {
...
func (q *pendingQueue) remove(addr common.Address, nonce uint64) {
	...
	if collection.len() == 0 {
		delete(q.txs, addr)
		heap.Remove(q.heap, collection.GetHeapIndex())
	} else {
		heap.Fix(q.heap, collection.GetHeapIndex())
	}
}
