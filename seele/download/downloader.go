...
func (d *Downloader) fetchHeight(conn *peerConn) (*types.BlockHeader, error) {
	head, _ := conn.Head()
	go conn.RequestHeadersByHashOrNumber(head, 0, 1, false)
	msg, err := conn.waitMsg(seele.BlockHeadersMsg, d.cancelCh)
	if err != nil {
		return nil, err
	}
...
func (d *Downloader) RegisterPeer(peerID string, peer *seele.Peer) {
	d.lock.Lock()
	defer d.lock.Unlock()
	newConn := newPeerConn(peer)
...
func (d *Downloader) peerDownload(peer *peerConn, tm *taskMgr) {
	defer d.sessionWG.Done()
	bMaster := (peer.PeerID == d.masterPeer)
	peerID := peer.PeerID
	var err error
outLoop:
	for !tm.isDone() {
		hasReqData := false
		if startNo, amount := tm.getReqHeaderInfo(peer); amount > 0 {
			hasReqData = true
			if err = peer.RequestHeadersByHashOrNumber(common.Hash{}, startNo, amount, false); err != nil {
				d.log.Info("RequestHeadersByHashOrNumber err!")
				break
			}
			msg, err := peer.waitMsg(seele.BlockHeadersMsg, d.cancelCh)
			if err != nil {
		...
		if startNo, amount := tm.getReqBlocks(peer); amount > 0 {
			hasReqData = true
			if err = peer.RequestBlocksByHashOrNumber(common.Hash{}, startNo, amount); err != nil {
				d.log.Info("RequestBlocksByHashOrNumber err!")
				break
			}
			msg, err := peer.waitMsg(seele.BlocksMsg, d.cancelCh)
			if err != nil {
